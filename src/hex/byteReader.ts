...
export class PagedByteReader implements vscode.Disposable {
  ...
  private async readPage(pageStart: number, length: number): Promise<Uint8Array> {
    ...
    const fullContent = await vscode.workspace.fs.readFile(this.uri);
    return fullContent.subarray(pageStart, Math.min(fullContent.length, pageStart + length));
  }

  private async getFileHandle(): Promise<FileHandle> {
    if (!this.fileHandlePromise) {
      this.fileHandlePromise = open(this.uri.fsPath, 'r');
    }
